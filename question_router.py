...
def _call_gemini(system_prompt: str, user_text: str) -> str | None:
    """Call Gemini once. Returns text, or None on any failure (-> fallback)."""
...
def answer_question(question: str, kundli: dict, name: str = "", gender: str = "") -> str:
    """
    ...
    # ── Hard guardrails: never call the LLM ─────────────────────────────────
    ...
    system  = _system_prompt(name, gender, kind if level == "soft" else "")

    user_text = (
        f"{context}\n\n"
        f"=== QUESTION ===\n{question}\n\n"
        "Answer as Hora, the Vedic astrologer, following all the rules."
    )

    # ── Call Gemini, else fall back ─────────────────────────────────────────
    reply = _call_gemini(system, user_text)
    if reply:
